...
import pandas as pd
from nfp.servicos.controles.divide_planilha import (
    finalizar_execucao,
    selecionar_execucao,
...
class DividePlanilha:
    ...
    def dividir_planilha(self):
        ...
        nome_plan = (
            nome_plan.replace(".xlsx", "").replace(".xls", "").replace(".csv", "")
        )
        ...
        while end_slice_index < len(self.planilha):
            ...
            nome_arquivo = "{}/Plan_{}_{}.{}".format(
                self.path_planilha_saida, num_planilha, nome_plan, extensao
            )
            ...
            num_planilha += 1
            logging.info(
                f"**********Linhas {start_slice_index + 1} a {end_slice_index} gravadas no arquivo {nome_arquivo}"
            )

    def _carrega_planilha(self):
        if "csv" in self.path_planilha_entrada:
            return pd.read_csv(
                self.path_planilha_entrada, encoding="utf8", header=self.header_plan
            )
    ...
    def retornar_resultado(self, retorno):
        ...
        abrir_popup(texto, 5, title="Retorno")
